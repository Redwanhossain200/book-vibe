    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] px-4 text-center">
        ...
        <p className="text-gray-500 max-w-md mb-8">
          The book you are looking for doesn't exist or has been removed from
          our library.
        </p>
        <button
          onClick={() => navigate('/')}
    ...
    <div className="container mx-auto my-6 px-4 md:my-12">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-base-100 rounded-xl">
        ...
        <div className="flex flex-col space-y-4 py-2">
          ...
          <p className="py-3 border-y border-gray-200 text-lg font-medium text-gray-600">
            {category}
          </p>
          ...
          <div className="flex flex-wrap items-center gap-3 py-2">
            ...
            {tags.map((tag, index) => (
              <span
                key={index}
                className="px-4 py-1 text-green-600 bg-green-50 rounded-full font-medium text-sm">
                # {tag}
              </span>
            ))}
          </div>
          ...
          <div className="flex items-center gap-4 pt-4">
            <button
              className="btn btn-outline border-gray-300 px-8 hover:bg-gray-800 rounded-2xl hover:text-white transition-colors"
              onClick={() => handleMarkAsRead(expectedBook)}>
              Mark as Read
            </button>
            <button
              className="btn bg-[#50B1C9] hover:bg-[#3e8ea2] border-none rounded-2xl hover:text-black text-white px-8"
